...
data = sc.textFile('/hw3data_original/*')
...
fnldata = data.map(lambda line: (line.split(':::')[1].split('::'), line.split(':::')[-1].lower()))\
		.map(lambda (auth, title): (auth, re.sub('\W+',' ',title)))\
		.flatMap(lambda (auths, title): [(auth, title) for auth in auths])\
		.sortByKey()\
		.flatMap(lambda (auth, title): [(auth,word) for word in title.split()])\
		.filter(lambda (auth, word): word not in allStopWords  and len(word)>1 )\
		.groupByKey().sortByKey()

for (auth, lst) in fnldata.collect():
	print('{}:{}'.format(auth.encode('utf-8'), Counter(lst).most_common()))
sc.stop()
