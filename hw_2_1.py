...
data = sc.wholeTextFiles('/shakespeare_original/*')
...
fnlrdd = data.map(lambda (filepath, contents): (filepath.split('/')[-1], re.sub('\W+',' ',contents)))\
		.flatMap(lambda (filename,contents): [(word, filename )for word in contents.lower().split()])\
		.groupByKey()


print('Wrting outputs to file')
outfile1 = open('HW_2_1_1_output.txt', 'w') # Output for HW Question 1 Part 1  
outfile2 = open('HW_2_1_2_output.txt', 'w') # Output for HW Question 1 Part 2
for (key, value) in fnlrdd.collect():
	vals = Counter(value)	
        outfile1.write('{}:{}\n'.format(key, vals.keys()))
        outfile2.write('{}:{}\n'.format(key, vals.most_common()))
outfile1.close()
outfile2.close()
